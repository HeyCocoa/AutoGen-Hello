...
2. 调用 web_search() 恰好 2 次，搜索：
   - 行业/市场最新情况（加上当前年份，如"2026年XX行业"）
   - 目标受众痛点或竞品做法（二选一，按场景最关键的来）
...
- 3-5 个核心选题方向（按优先级排序）
- 每个方向的切入角度
- **具体的内容形式建议**（如：短视频、深度文章、白皮书、案例集、直播等）
- 应该避免的坑
...
【重要：保留来源 URL】
在报告末尾必须添加"## 参考来源"部分，列出搜索结果中的所有 URL 链接：
- 格式：`- [标题](URL)`
- 直接从搜索结果的【来源】部分复制，不要编造
- 至少列出 5 个来源
...
- web_search(query): 搜索验证信息或寻找反例（最多调用 1 次）
...
- 如需搜索，请把所有要验证的点合并进一次查询
- 重点关注那些会影响最终策略的关键问题
- **不要输出"打回"或"通过"，只输出质检报告内容**
"""
...
> 生成时间：[使用分析师报告中的日期，格式：YYYY-MM-DD]
> 数据来源：联网搜索
...
## 参考来源
【重要】从分析师和批评者的输出中找到所有【来源】部分，提取其中的 URL 链接：
- 搜索输出中会有类似 `[标题](https://xxx.com)` 的格式，直接复制这些链接
- 必须是真实的 URL（以 http:// 或 https:// 开头），不要编造
- 至少列出 5 个不同的来源链接
- 示例格式：
  - [2026年猫粮市场趋势报告](https://www.example.com/report)
  - [小红书宠物食品种草分析](https://zhuanlan.zhihu.com/p/xxx)
...
3. 关键词是否具体可检索（不要求时间口径完全一致，只要能搜到结果即可）
...
注意：只有在关键维度缺失或关键词完全无法检索时才打回，不要因为小问题（如时间表述不统一）打回。
"""
...
如果需要验证某些信息，可以使用 web_search 工具（最多调用 2 次）。

【重要】只输出一份【质检报告】，不要输出"打回"或"通过"。
"""
