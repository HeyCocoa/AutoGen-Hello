...
- `app/prompts.py` 集中管理所有 Agent 的 system_message 和任务提示词模板。
- `app/tools.py` 提供 Agent 可调用的工具函数（日期、联网搜索、计算）。
- `output/` 为运行期生成物（策略 Markdown 等），视为产物目录而非源码。
- 依赖在 `requirements.txt`，架构说明见 `CLAUDE.md` 与 `README.md`。

## 构建、测试与开发命令
- `pip install -r requirements.txt`：安装依赖（包括 zai-sdk 用于智谱联网搜索）。
...
- **Prompt 集中放在 `app/prompts.py`**，Agent 文件只负责创建实例。
- 终端显示与流式控制集中在 `app/utils/`。
...
- 支持智谱AI联网搜索：设置 `ZHIPU_WEB_SEARCH_ENABLED=true` 启用。
